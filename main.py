...
def get_resumen(
    ...
    cids = [int(x) for x in (clinicas_ids or "").split(",") if x.strip().isdigit()]
    if clinica: cids.append(clinica)
    if cids:
        if clinicas is not None: cids = [c for c in cids if c in clinicas]
        if cids:
            phs = ",".join(["?"]*len(cids)); where += f" AND clinica IN ({phs})"; params.extend(cids)
    mids = [int(x) for x in (mutuales_ids or "").split(",") if x.strip().isdigit()]
    if mutual: mids.append(mutual)
    if mids:
        phs = ",".join(["?"]*len(mids)); where += f" AND mutual IN ({phs})"; params.extend(mids)
...
def get_analisis_practicas(
    ...
    if clinica:
        cids.append(clinica)
    if cids:
        if clinicas_usuario is not None:
            cids = [c for c in cids if c in clinicas_usuario]
        if cids:
            phs = ",".join(["?"] * len(cids)); where += f" AND clinica IN ({phs})"; params.extend(cids)
    # Multi-mutual
    mids = []
